...
def plot_metric(dataset_name, dataset_results, metric, save_path=None):
    ...
    if '@' in metric_name:
        metric_name = METRIC_DISPLAY_NAMES[metric_name.split('@')[0]] + "@" + metric_name.split('@')[1]
    else:
...
def plot_metrics(dataset_name, dataset_results, metrics, save_path=None):
    ...
    for model_index, (model_name, model_results) in enumerate(dataset_results.items()):
        for reranker_index, (reranker_name, metric_values) in enumerate(model_results.items()):
            values = [metric_values[metric] for metric in metrics]
            bar_position = x[model_index + 1] - (reranker_index - len(model_results)/2) * bar_width

            bar = None
            for metric_index, (value, metric_name) in enumerate(zip(values, metric_names)):
                ...
                bar = ax.barh(
                    ...
                    value - left,
                    height=bar_width,
                    label=DISPLAY_NAMES[reranker_name] if model_index == 0 and metric_index == 0 else "",
            ...
            ax.bar_label(bar, labels=[f"{values[-1]:.3f}"], label_type="edge", padding=3)

    legend1 = ax.legend(title="Rerankers", loc='upper left')

    ax.legend(title="Metrics", loc='upper right', handles=[
            Patch(
                facecolor='white',
                hatch=hatch,
                edgecolor="black",
...
if __name__ == "__main__":
    ...
    for dataset_name, dataset_results in eval_results.items():
        for metric in evaluate.METRICS_PER_DATASET[dataset_name]:
             plot_metric(dataset_name, dataset_results, metric, save_path=save_path)
        plot_metrics(dataset_name, dataset_results, [evaluate.Success@1, evaluate.Success@5, evaluate.Success@10], save_path=save_path)