...
\begin{figure}[H]
  \makebox[\textwidth][c]{
    \includegraphics[width=0.9\paperwidth]{AIR_schematic.pdf}
  }
...
\section{Dataset}
As citation relationships between research papers are a strong indicator of relevance, we will utilize the SciDocs Cite
...
fine-tuning. The training set for fine-tuning will be split further into a training and validation set to monitor for
overfitting during training and to perform hyperparameter tuning. For the evaluation and the test splits we will make
sure that the set of contained documents (the set of queries and the set of positive and negative samples) are disjoint
from the training set to avoid data leakage.

\section{Evaluation and Metrics}
All models will be evaluated on the validation set of the SciDocs Cite Prediction dataset and the RELISH dataset using
...
\section{Models, Training and Comparison}
...
and non-relevant documents for a given query using binary cross entropy loss. Given two different documents (title +
abstract) inputs, the cross encoder model will return a value $\in [0, 1]$ indicating how relevant one document is to
the other. The following combinations of embedding models and re-ranking models from HuggingFace will be evaluated
(subject to change during implementation), as shown in Table 1.
