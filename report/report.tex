...
\begin{abstract}
  ...
  approximate k-nearest results) in the context of recommending research papers.\\ Our overarching research question is:
...
document-level embeddings of scientific papers based on the titel and abstract. Singh et
...
GPU compute, we fine-tune for one full epoch with a batch size of 40, 16-bit precision, learning-rate warmup and all
...
directly ouputs the similarity between the query and other documents, as it processes both texts jointly.
...
model. If a re-ranking model is used, these documents are re-ranked by using the predicted relevance between the query
...
For each query, we retireve the top-k (optionally re-ranked) documents. This is repeated for every combination of our
...
This suggests that the fine-tuned reranking model focuses more on ranking the top documents correctly, while the
...
RELISH dataset is generally higher than on the SciDocs-Cite dataset. This supports the effectiveness of pre-training on
citation based datasets for research paper recommendation tasks. But is also an effect of the different dataset
characteristics, such as the number of relevant documents per query and the domain specificity.
...
\begin{table}[ht]
  ...
  \begin{adjustbox}{max width=\paperwidth - 0.5in,center}
    ...
    \begin{tabular}{|l|l|}
      \hline
      \textbf{Resource Name}         & \textbf{Hugging Face Link}                                                          \\
      \hline
      \hline
