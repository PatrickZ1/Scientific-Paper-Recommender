...
We are using the citation-triplets subset (also called SciDocs-Cite) from SciRepEval~\cite{Singh2022SciRepEvalAM} for
...
expert-annotated relevance judgments for research paper recommendations in the biomedical domain. It contains around
...
remove all papers in its training set that are also present in the RELISH dataset.
...
library~\cite{zhu2004recall,jarvelin2002cumulated}. These metrics are calculated at multiple cutoff values
...
RELISH dataset is generally higher than on the SciDocs-Cite dataset. While this supports the effectiveness of
...
\hline
 ...
 \hline
 \end{tabular}
 \caption{Evaluation results on the SciDocs-Cite citation-triplets dataset for all model combinations and selected metrics including the 90\% confidence interval. Best results per metric are \textbf{bolded}, second best are \underline{underlined}.}
...
\begin{table}[ht]
  ...
  \begin{adjustbox}{max width=\paperwidth - 0.5in,center}
    ...
    \begin{tabular}{|l|l|}
      ...
      \hline
      SciDocs-Cite & \url{https://huggingface.co/datasets/allenai/scirepeval/viewer/cite_prediction_new} \\
      \hline
